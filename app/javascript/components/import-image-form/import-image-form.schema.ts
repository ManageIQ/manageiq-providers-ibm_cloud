...
const fieldsForPVC = (
  ...
  providers: Promise<OptionType[]>,
  storages: Promise<OptionType[]>,
  diskTypes: Promise<OptionType[]>,
  images: Promise<OptionType[]>,
  buckets: Promise<OptionType[]>,
): SchemaField[] => [
  {
    ...
    onChange: (value) => {
      setState({ ...state, src_provider_id: value as string });
    },
  },
  {
    component: componentTypes.SELECT,
  ...
  },
  {
    ...
    onChange: (value) => {
      setState({ ...state, obj_storage_id: value as string });
    },
  },
  {
    component: componentTypes.SELECT,
  ...
  },
  {
    ...
    options: Array.from(Array(24).keys()).map((h) => {
      return { label: h + 1 + " hours", value: h + 1 };
    }),
  },
  {
...
const defaultFields = (
...
): SchemaField[] => [
  {
    ...
    onChange: (value) => {
      setState({ ...state, provider_type: value as string });
    },
  },
];
